...
(such as rtcwmain + missionpack content combination in a mod for instance)
BASEGAME is the hardcoded base game ("rtcwmain")
...
typedef struct
{
	char pakPathname[MAX_OSPATH];               ///< c:\\etlegacy\\rtcwmain
	char pakFilename[MAX_OSPATH];               ///< c:\\etlegacy\\rtcwmain\\pak0.pk3
	char pakBasename[MAX_OSPATH];               ///< pak0
	char pakGamename[MAX_OSPATH];               ///< rtcwmain
...
typedef struct
{
	...
	char fullpath[MAX_OSPATH];      ///< c:\\etlegacy\\rtcwmain
	char gamedir[MAX_OSPATH];       ///< rtcwmain
} directory_t;
...
/**
 * @brief A mod directory is a peer to rtcwmain with a pk3 in it
...
void FS_AddGameDirectory(const char *path, const char *dir)
{
	...
	while ((pakfilesi < numfiles) || (pakdirsi < numdirs))
	{
		...
		else
		{
			...
			Q_strncpyz(search->dir->path, curpath, sizeof(search->dir->path));  // c:\etlegacy\rtcwmain
			Q_strncpyz(search->dir->fullpath, pakfile, sizeof(search->dir->fullpath));  // c:\etlegacy\rtcwmain\mypak.pk3dir
...
const char *FS_ReferencedPakNames(void)
{
	...
	// and referenced one's from rtcwmain
	for (search = fs_searchpaths ; search ; search = search->next)
	{
		// is the element a pak file?
		if (search->pack)
...
static void FS_CheckRequiredFiles(int checksumFeed)
{
	if (!FS_FOpenFileRead("pak0.pk3", NULL, qfalse))
	{
		...
		Com_Error(ERR_FATAL, "FS_InitFilesystem: Original game data files not found.\n\nPlease copy pak0.pk3 from the 'rtcwmain' path of your Wolfenstein: Enemy Territory installation to:\n\n\"%s%c%s\"\n\n",
...
void FS_InitFilesystem(void)
{
	...
	if (!strcmp(tmp_fs_game->string, ""))
	{
		...
		Com_Printf("INFO: fs_game now defaults to '%s' mod instead of 'rtcwmain'\n", tmp_fs_game->string);
	}
