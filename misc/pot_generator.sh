...
CLIENT_POT=main/locale/etlegacy_client.pot
MOD_POT=main/locale/etlegacy_mod.pot
...
# MOD
touch ${MOD_POT}
find src/game src/ui src/cgame main/ui -name '*.c' -o -name '*.cpp' -o -name '*.menu' | sort | xgettext \
...
# Update po files
for PO in $(find main/locale/mod -type f -name "*.po"); do msgmerge -vU ${PO} ${MOD_POT}; done
for PO in $(find main/locale/client -type f -name "*.po"); do msgmerge -vU ${PO} ${CLIENT_POT}; done
