...
CLIENT_POT=rtcwmain/locale/etlegacy_client.pot
MOD_POT=rtcwmain/locale/etlegacy_mod.pot
...
# MOD
touch ${MOD_POT}
find src/game src/ui src/cgame rtcwmain/ui -name '*.c' -o -name '*.cpp' -o -name '*.menu' | sort | xgettext \
...
# Update po files
for PO in $(find rtcwmain/locale/mod -type f -name "*.po"); do msgmerge -vU ${PO} ${MOD_POT}; done
for PO in $(find rtcwmain/locale/client -type f -name "*.po"); do msgmerge -vU ${PO} ${CLIENT_POT}; done
